...
impl AudioControl for UnixController {
    ...
    fn set_master_volume_by_index(&mut self, device_index: u32, volume: u16) {
        self.controller.set_device_volume_by_index(device_index, ChannelVolumes::default().set(2, Volume(UnixController::map(volume))))
    }

    fn set_master_volume_by_name(&mut self, device_name: &str, volume: u16) {
        self.controller.set_device_volume_by_name(device_name, ChannelVolumes::default().set(2, Volume(UnixController::map(volume))))
    }
...
fn set_app_volume(controller: &mut SinkController, app: ApplicationInfo, volume: u16) {
    ...
    trace!("(app: {:?}) left: {:?}, right: {:?}, change: {}", app.name, app.volume.get()[0], app.volume.get()[1], volume_change);
    if volume_change > 0. {
